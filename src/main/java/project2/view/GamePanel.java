...
public class GamePanel extends JPanel implements GameEventListener {
    ...
    private void toggleSavePopup() {
        ...
        if (newSlot != -1) {
            ...
            JButton newBtn = new JButton("New Slot " + finalNewSlot);
            newBtn.addActionListener(e -> {
                controller.saveGame(finalNewSlot);
                savePopup.setVisible(false);
            });
        ...
        mainMenuBtn.addActionListener(e -> {
            savePopup.setVisible(false);
            window.showPanel("MAIN_MENU");
        });
        savePopup.add(mainMenuBtn);
    ...
    public void addNotify() {
        super.addNotify();
        if (controller.getCurrentScene() == null) {
            return;
        }
        updateScene();
        updateChoices();
